...
ui <- fluidPage(
  titlePanel('Plot'),
  # 4. Display interactive plot using `plotlyOutput()`
  plotlyOutput('plot')
)
server <- function(input, output, session){
  # 1. Render interactive plot using `renderPlotly()` and 
  # 2. Assign it to `output$plot`
...
ui <- fluidPage(
  ...
  textInput('name', 'Enter Name'),
  plotOutput('plot'),
  # Display an interactive table output of trends
  DT::DTOutput('table')
)
...
⚽ opy the code for the app we just created and make two modifications:
